...
$\av_p$, we can write $\U = \V \A$.  Similarly, there is a matrix
$\A'$ such that $\V = \U \A'$.  Putting these together, we get

$$
%
\begin{align*}
\U = \V \A = \U \A' \A \Rightarrow \id_Q = \A' \A \Rightarrow \A' = \A^{-1}.
\end{align*}
%
$$

Furthermore, using the fact that $\U$ and $\V$ are orthonormal,

$$
\A = \V^\trans \U \Rightarrow \A^{-1} = \U^{-1} (\V^\trans)^{-1} = \U^\trans \V = (\U \V^\trans)^\trans = \A^\trans,
$$
