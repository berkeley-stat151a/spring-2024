...
\\={}&
\meann \res_n^2 + 2 (\beta - \betahat)^\trans \meann \xv_n  + 
  (\beta - \betahat)^\trans \meann \xv_n \xv_n^\trans (\beta - \betahat).
\end{aligned}
$$

By A3, both $\meann \xv_n$ and $\meann \xv_n \xv_n^\trans$ converge to
something bounded, and we showed above that $\beta - \betahat \rightarrow 0$,
and that $\meann \res_n^2 \rightarrow \sigma^2$ by the LLN.  It follows
that
...
We can in fact get a more precise estimate by writing $\sigmahat^2$ in matrix
...
\expect{\trace{\proj{\X^\perp}  \resv^\trans \resv}}
\\={}&
\trace{\proj{\X^\perp}  \expect{\resv^\trans \resv}}
\\={}&
